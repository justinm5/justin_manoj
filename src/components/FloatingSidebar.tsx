...
export const FloatingSidebar = () => {
  return (
    <>
      <div className="hidden md:block fixed left-4 top-6 z-40">
        <div className="flex flex-col gap-3">
          <div className="rounded-2xl border border-border/50 bg-background/70 backdrop-blur-2xl p-3 shadow-[0_20px_44px_rgba(0,0,0,0.25)]">
            <nav className="flex flex-col gap-1 text-[11px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
              {navLinks.map((item) => (
                <NavLink
                  key={item.href}
                  to={item.href}
      ...
      <div className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-40">
        <div className="rounded-2xl border border-border/50 bg-background/80 backdrop-blur-2xl px-4 py-3 shadow-[0_18px_36px_rgba(0,0,0,0.25)]">
          <div className="flex items-center justify-center gap-3 text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
            {navLinks.map((item, index) => (
              <div key={item.href} className="flex items-center gap-3">
                ...
                {index === 0 ? <span className="h-3 w-px bg-border/60" /> : null}
              </div>
            ))}
          </div>
            ...
            {socialLinks.map((social) => {
              ...
              return (
                <a
                  ...
                  rel={isEmailLink ? undefined : "noopener noreferrer"}
                  aria-label={social.label}
                  className="rounded-2xl border border-border/40 p-2 bg-card/20 backdrop-blur-sm transition-colors hover:border-accent/50"
                >
